...
class DL19Reranker:
    ...
    def run_reranking(self):
        ...
        logger.info(f"Loaded {len(queries)} queries from topics")
        logger.info(f"Loaded {len(baseline_run)} queries from baseline run")
        if queries: logger.info(f"Sample query ID from topics: '{list(queries.keys())[0]}'")
        if baseline_run: logger.info(f"Sample query ID from baseline: '{list(baseline_run.keys())[0]}'")
        ...
        # Final Evaluation
        logger.info("========================================================")
        logger.info("DL 19 RERANKING RESULTS (Threshold rel=2 for MAP/MRR)")
        logger.info("========================================================")
        ...
        with open(output_path, 'w') as f:
            for qid in sorted(final_run_results.keys()):
                for rank, (docid, score) in enumerate(final_run_results[qid], start=1):
                    f.write(f"{qid} Q0 {docid} {rank} {score:.6f} MS-MEQE-DL19-Rerank\n")

...
Key Features:
  - 5-Fold GridSearchCV for hyperparameter tuning (optional via --tune)
...
# --- FEATURE SCHEMAS ---
VALUE_FEATURE_NAMES = [
    'cos_sim_q', 'cos_sim_pseudo', 'l2_dist_q',  # Semantic
    'idf', 'tf_pseudo', 'rm3_score', 'coverage_pseudo',  # Statistical
    'in_query', 'jaccard', 'pmi_max',  # Interaction
...
WEIGHT_FEATURE_NAMES = [
    'cos_sim_q', 'l2_dist_q', 'edit_dist',  # Drift
    'df_norm', 'cf_norm', 'idf', 'polysemy', 'coverage',  # Ambiguity
    'source_docs', 'source_kb', 'source_emb',  # Source
    'native_rank', 'native_score', 'source_confidence', 'rm3_score',
    'term_len_norm', 'is_stopword', 'in_query', 'char_diversity', 'has_numbers'
]
...
def train_value_model(
    ...
    if do_tuning:
        logger.info("Starting 5-Fold Cross-Validation for Hyperparameter Tuning...")
        param_grid = {
...
def main():
    ...
    logger.info(f"Tuning mode: {args.tune}")
    logger.info("")
    ...
    value_model = train_value_model(
        ...
        output_path=str(output_dir/"value_model.pkl"),
    ...
    weight_model = train_weight_model(
        ...
        output_path=str(output_dir / "weight_model.pkl"),
        n_estimators=args.n_estimators,
        max_depth=args.max_depth,
        learning_rate=args.learning_rate,
        reg_alpha=args.reg_alpha,

...
def read_docs(doc_file):
    docs = {}
    with open(doc_file, 'r') as f:
        for line in tqdm(f, total=351247):
            d = json.loads(line)
            docs[d['entity_id']] = d['wikipedia_lead_text']
    return docs
    ...
    try:
        ...
        payload = [("gcube-token", MY_GCUBE_TOKEN),
                   ...
                   ("method",
                    ...
                    "centroid:rescore=true,topk:k=5,voting:relatedness=lm,ranker:models=0046.models,"
                    "confidence:models=pruner-wiki.linear")]

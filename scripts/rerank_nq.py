...
class NQReranker:
    ...
    def run_reranking(self):
        ...
        # Final Evaluation
        logger.info("========================================================")
        logger.info("NQ RERANKING RESULTS")
        logger.info("========================================================")
        
        eval_dict = {qid: {d: s for d, s in res} for qid, res in final_run_results.items()}
        evaluator = pytrec_eval.RelevanceEvaluator(qrels, {'map', 'ndcg_cut_10', 'recip_rank', 'recall_100', 'recall_1000'})
        ...
        avg_recall_100 = statistics.mean([v['recall_100'] for v in final_metrics.values()])
        avg_recall_1000 = statistics.mean([v['recall_1000'] for v in final_metrics.values()])
        
        logger.info(f"Average MAP:          {avg_map:.4f}")
        logger.info(f"Average nDCG@10:      {avg_ndcg:.4f}")
        logger.info(f"Average MRR:          {avg_mrr:.4f}")
        logger.info(f"Average Recall@100:   {avg_recall_100:.4f}")
        ...
        with open(output_path, 'w') as f:
            for qid in sorted(final_run_results.keys()):
                for rank, (docid, score) in enumerate(final_run_results[qid], start=1):
                    f.write(f"{qid} Q0 {docid} {rank} {score:.6f} MS-MEQE-NQ-Rerank\n")
...
def main():
    ...
    parser.add_argument("--collection-size", type=int, default=2681468)
    
    args = parser.parse_args()
    
    reranker = NQReranker(args)
